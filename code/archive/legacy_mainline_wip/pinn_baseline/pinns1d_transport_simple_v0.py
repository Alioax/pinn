...
sampler2d = qmc.LatinHypercube(d=1,seed=1234567) #

x_star = sampler2d.random(n=num_collocation) # np.random.random(num_collocation)
t_star = sampler2d.random(n=num_collocation)* t_final_star #np.random.random(num_collocation) * t_final_star

x_star_init = sampler2d.random(n=num_ic)  # np.random.random(num_ic)
t_star_init = np.zeros_like(x_star_init)

x_star_in = np.zeros(num_bc)
t_star_in = sampler2d.random(n=num_bc)* t_final_star #np.random.random(num_bc) * t_final_star

x_star_out = np.ones(num_bc)
t_star_out = sampler2d.random(n=num_bc) * t_final_star #np.random.random(num_bc) * t_final_star
...
def closure():
   ...
   # C_BL_f = 1-torch.exp(-10*train_t_in)
    ...
    # dC_BR_pred_dx = gradients(C_BR_pred, train_x_out)
    # out_loss = ((dC_BR_pred_dx)**2).mean() #  
...
plt.savefig("Fig_Ana&Pinns_Transp.jpg")
plt.show()
# ============================================================================
...
plt.savefig("Fig_coll_points.jpg")
