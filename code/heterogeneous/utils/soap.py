# MIT License — from https://github.com/nikhilvyas/SOAP (Vyas et al., arXiv:2409.11321)
# Copied as a single-file drop-in for PINO heterogeneous training experiments.
...
class SOAP(optim.Optimizer):
                ...
                for idx, sh in enumerate(new_grad.shape):
                    if sh <= max_precond_dim:
                        outer_product = torch.tensordot(
                            new_grad,
                            new_grad,
                            dims=[[*chain(range(idx), range(idx + 1, len(new_grad.shape)))]]
                            * 2,
                        )
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        ...
        matrix = []
        orth_matrix = []
        for m, o in zip(precond_list, orth_list, strict=True):
            if len(m) == 0:
                matrix.append([])
                orth_matrix.append([])
                continue
        ...
        final = []
        for ind, (m, o) in enumerate(zip(matrix, orth_matrix, strict=True)):
            if len(m) == 0:
                final.append([])
                continue
