"""
PINN Parameter Study for 1D Contaminant Transport

This script is based on the baseline PINN implementation and is used to study
the effect of different dispersion coefficients (D) and advection speeds (U)
on the PINN solution.
...
D = 5e-7 * 86400          # m²/day (dispersion coefficient)
...
# Plotting parameters
times_days = [200, 400, 600, 800, 1000]  # times to plot (days)
...
def plot_concentration_profiles(model):
    ...
    plot_path = results_dir / 'pinn_parameter_study_concentration_profiles.png'
    plt.savefig(str(plot_path), dpi=300, bbox_inches='tight')
    print(f"Plot saved to: {plot_path}")
    
    # Save PDF copy to report 1 LaTeX figs (local sources under docs/reports/_src/)
    report_figs_dir = script_dir.parent.parent.parent / 'docs' / 'reports' / '_src' / 'report-1-pinn-baseline' / 'figs'
