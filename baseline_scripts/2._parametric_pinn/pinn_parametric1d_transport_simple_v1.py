...
"""
Parametric PINN for 1D Contaminant Transport (dimensionless).

Same PDE as baseline but the network learns C* = f(x*, t*, log Pe) over a range of
Péclet numbers:
    ∂C*/∂t* + ∂C*/∂x* - (1/Pe) ∂²C*/∂x*² = 0
...
# ============================================================================
# Neural Network Architecture (3 inputs: x*, t*, log Pe)
# ============================================================================

class PINN_Transp_Param(nn.Module):
    """Neural network: (x*, t*, log Pe) -> dimensionless concentration C*."""

    def __init__(self, num_layers, num_neurons, activation):
        super(PINN_Transp_Param, self).__init__()
        layers = [nn.Linear(3, num_neurons), activation()]
...
model = PINN_Transp_Param(num_layers, num_neurons, activation).to(device)
model.init_weights()
...
for pe_val in pe_values_to_plot:
    ...
    fig.suptitle('Parametric PINN: C* vs x* (Pe = %g)' % pe_val, fontsize=14)
