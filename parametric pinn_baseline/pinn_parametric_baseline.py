"""
Parametric PINN for 1D Advection-Dispersion
...
Pe_min = 10              # minimum Peclet number for training
...
# Plotting parameters
times_days = [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000, 2500]  # times to plot (days)
...
# log10Pe_min = np.log10(Pe_min)
# log10Pe_max = np.log10(Pe_max)
# order_of_magnitude_delta = log10Pe_max - log10Pe_min
# Pe_scaler = order_of_magnitude_delta ** (- 1)
# Pe_min_scaled = (log10Pe_min - log10Pe_min) * Pe_scaler
# Pe_max_scaled = (log10Pe_max - log10Pe_min) * Pe_scaler
# logPe_min = Pe_min_scaled
# logPe_max = Pe_max_scaled
...
collocation_sample = sample_collocation_points()
x_star_pde = collocation_sample["x_star_pde"]
t_star_pde = collocation_sample["t_star_pde"]
log_pe_pde = collocation_sample["log_pe_pde"]
x_star_ic = collocation_sample["x_star_ic"]
t_star_ic = collocation_sample["t_star_ic"]
x_star_inlet = collocation_sample["x_star_inlet"]
t_star_inlet = collocation_sample["t_star_inlet"]
x_star_outlet = collocation_sample["x_star_outlet"]
t_star_outlet = collocation_sample["t_star_outlet"]
...
# Plot parametric coverage (t* vs log Pe)
plt.figure(figsize=(6.5, 4))
...
def compute_losses(samples):
    ...
    # pe_pde = 10 ** ((log_pe_pde_tensor * Pe_scaler) + log10Pe_min)
...
plot_path = results_dir / f'loss.png'
plt.savefig(str(plot_path), dpi=plot_dpi, bbox_inches='tight')
    

for pe_plot in [10, 50, 1e2, 500, 1e3, 1e4, 1e5]:

    log_pe_plot = np.log(pe_plot)
    # Map the chosen Pe to a physical dispersion coefficient for comparison
    D_plot = U * L / pe_plot
    ...
    for idx, t_days in enumerate(reversed(times_days)):
        ...
        C_analytical = analytical_solution(x_plot, t_days, U_param=U, D_param=D_plot, C_0_param=C0)
    ...
    plot_path = results_dir / f'{pe_plot} pinn_baseline_concentration_profiles.png'
    plt.savefig(str(plot_path), dpi=plot_dpi, bbox_inches='tight')
