...
Pe_min = 1e-1              # minimum Peclet number for training
...
log_pe_pde_tensor = torch.tensor(log_pe_pde, requires_grad=True)
x_star_ic_tensor = torch.tensor(x_star_ic, requires_grad=True)
t_star_ic_tensor = torch.tensor(t_star_ic, requires_grad=True)
log_pe_ic_tensor = torch.tensor(log_pe_ic, requires_grad=True)
...
log_pe_inlet_tensor = torch.tensor(log_pe_inlet, requires_grad=True)
...
log_pe_outlet_tensor = torch.tensor(log_pe_outlet, requires_grad=True)
...
def closure():
    ...
    log_pe_pde_grad = log_pe_pde_tensor.clone().detach().requires_grad_(True)
    C_star_pde = model(x_star_pde_grad, t_star_pde_grad, log_pe_pde_grad)
    ...
    d2C_dx2_star = grad(dC_dx_star, x_star_pde_grad, grad_outputs=torch.ones_like(dC_dx_star),
                        create_graph=True, retain_graph=True)[0]
    pe_pde = torch.exp(log_pe_pde_grad)
